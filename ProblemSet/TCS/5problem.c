...
10 -> Value of N
[a,a,a,b,b,b,c,c,c,a,f,c] -> B[], elements B[0] to B[N-1] where input each element is separated by new line.
Output :
...
int main(){
    int arr[12]={'a','a','a','b','b','b','c','c','c','a','f','c'};
    int counter=0;
    for(int i=0;i<12;i++){
    ...
    printf("Number of counts: %d",counter);
    return 0;
}