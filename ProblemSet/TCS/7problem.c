...
int main(){
    ...
    int meanOxygen=0;
    int maxMinOxygen=0;
    int traineeQualified;
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            printf("Enter the Oxygen Level for round %d and for trainee %d",j+1,i+1);
            scanf("%d",&trainee[i][j]);
            meanOxygen=meanOxygen+trainee[i][j];
        }
        meanOxygen=(meanOxygen/3);
        if(maxMinOxygen<meanOxygen){
            maxMinOxygen=meanOxygen;
            traineeQualified=i;
        }
    }
    if(maxMinOxygen*100 >=70){
        printf("Trainee NUmber %d Qualified",traineeQualified);
        return 0;
    } 
